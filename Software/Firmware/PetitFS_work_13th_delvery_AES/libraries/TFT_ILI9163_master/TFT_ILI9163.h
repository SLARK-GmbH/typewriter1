...
// MJS: add command defs for scrolling


#define ILI9163_NORON 	0X13 		//NORMAL DISPLAY MODE
#define ILI9163_VSCRSADD 0x37		//Vertical Scrolling Start Address
#define ILI9163_VSCRDEF	0x33		// Set v scroll window
...
class TFT_ILI9163 : public Print {
 ...
 public:
  ...
  int16_t  drawChar(unsigned int uniCode, int x, int y, int font),
    ...
    void   vScroll(uint8_t vsp),
	   exitScrollMode();

    int16_t setScrollWindow(uint8_t TopFixHeight, uint8_t BotFixHeight); // returns VScrollHeight
