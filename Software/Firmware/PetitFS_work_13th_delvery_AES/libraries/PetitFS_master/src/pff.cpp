...
#if _FS_FAT16
	case FS_FAT16 :
		if (disk_readp(buf, fs->fatbase + clst / 256, ((PFS_UINT)clst % 256) * 2, 2)) break;
		return LD_WORD(buf);
#endif
#if _FS_FAT32
	case FS_FAT32 :
		if (disk_readp(buf, fs->fatbase + clst / 128, ((PFS_UINT)clst % 128) * 4, 4)) break;
...
static
PFS_DWORD clust2sect (	/* !=0: Sector number, 0: Failed - invalid cluster# */
...
)
{
	...
	return (PFS_DWORD)clst * fs->csize + fs->database;
}


static
CLUST get_clust (
	PFS_BYTE* dir		/* Pointer to directory entry */
)
{
	FATFS *fs = FatFs;
	CLUST clst = 0;
...
FRESULT dir_find (
	...
	PFS_BYTE *dir		/* 32-byte working buffer */
)
{
	FRESULT res;
	PFS_BYTE c;
...
{
	PFS_BYTE c, ni, si, i, *sfn;
	const char *p;
#if _USE_LCC
#ifdef _EXCVT
	static const PFS_BYTE cvt[] = _EXCVT;
#endif
#endif
...
)
{
	...
	if ((PFS_BYTE)*path < ' ') {			/* Null path means the root directory */
		res = dir_rewind(dj);
		dir[0] = 0;
...
static
PFS_BYTE check_fs (	/* 0:The FAT boot record, 1:Valid boot record but not an FAT, 2:Not a boot record, 3:Error */
	PFS_BYTE *buf,	/* Working buffer */
	PFS_DWORD sect	/* Sector# (lba) to check if it is an FAT boot record or not */
)
{
...
{
	PFS_BYTE fmt, buf[36];
	PFS_DWORD bsect, fsize, tsect, mclst;
...
FRESULT pf_read (
	...
	PFS_UINT btr,		/* Number of bytes to read */
	PFS_UINT* br		/* Pointer to number of bytes read */
)
{
	DRESULT dr;
	CLUST clst;
	PFS_DWORD sect, remain;
	PFS_UINT rcnt;
	PFS_BYTE cs, *rbuff = (PFS_BYTE*)buff;  // whg
	FATFS *fs = FatFs;
	...
	*br = 0;
	...
	if (btr > remain) btr = (PFS_UINT)remain;			/* Truncate btr by remaining bytes */

	while (btr)	{									/* Repeat until all data transferred */
		if ((fs->fptr % 512) == 0) {				/* On the sector boundary? */
			cs = (PFS_BYTE)(fs->fptr / 512 & (fs->csize - 1));	/* Sector offset in the cluster */
		...
		rcnt = 512 - (PFS_UINT)fs->fptr % 512;			/* Get partial sector data from sector buffer */
		if (rcnt > btr) rcnt = btr;
		dr = disk_readp(!buff ? 0 : rbuff, fs->dsect, (PFS_UINT)fs->fptr % 512, rcnt);
...
FRESULT pf_write (
	...
	PFS_UINT btw,			/* Number of bytes to write (0:Finalize the current write operation) */
	PFS_UINT* bw			/* Pointer to number of bytes written */
)
{
	CLUST clst;
	PFS_DWORD sect, remain;
	const PFS_BYTE *p = (PFS_BYTE*)buff;  // whg
	PFS_BYTE cs;
	PFS_UINT wcnt;
	FATFS *fs = FatFs;
	...
	*bw = 0;
	...
	remain = fs->fsize - fs->fptr;
	if (btw > remain) btw = (PFS_UINT)remain;			/* Truncate btw by remaining bytes */

	while (btw)	{									/* Repeat until all data transferred */
		if ((PFS_UINT)fs->fptr % 512 == 0) {			/* On the sector boundary? */
			cs = (PFS_BYTE)(fs->fptr / 512 & (fs->csize - 1));	/* Sector offset in the cluster */
			if (!cs) {								/* On the cluster boundary? */
				if (fs->fptr == 0)					/* On the top of the file? */
					clst = fs->org_clust;
		...
		wcnt = 512 - (PFS_UINT)fs->fptr % 512;			/* Number of bytes to write to the sector */
		if (wcnt > btw) wcnt = btw;
		if (disk_writep(p, wcnt)) ABORT(FR_DISK_ERR);	/* Send data to the sector */
		fs->fptr += wcnt; p += wcnt;				/* Update pointers and counters */
		btw -= wcnt; *bw += wcnt;
		if ((PFS_UINT)fs->fptr % 512 == 0) {
...
FRESULT pf_lseek (
	PFS_DWORD ofs		/* File pointer from top of file */
)
{
	CLUST clst;
	PFS_DWORD bcs, sect, ifptr;
	FATFS *fs = FatFs;
	...
	fs->fptr = 0;
	if (ofs > 0) {
		bcs = (PFS_DWORD)fs->csize * 512;	/* Cluster size (byte) */
		if (ifptr > 0 &&
