...
void xmit_spi (PFS_BYTE d);		/* usi.S: Send a byte to the MMC */
PFS_BYTE rcv_spi (void);		/* usi.S: Send a 0xFF to the MMC and get the received byte */
...
static
PFS_BYTE send_cmd (
	PFS_BYTE cmd,		/* 1st byte (Start + Index) */
	PFS_DWORD arg		/* Argument (32 bits) */
)
{
	PFS_BYTE n, res;
  ...
  spi_set_divisor(CardType);  // whg
	...
	xmit_spi((PFS_BYTE)(arg >> 24));		/* Argument[31..24] */
	xmit_spi((PFS_BYTE)(arg >> 16));		/* Argument[23..16] */
	xmit_spi((PFS_BYTE)(arg >> 8));			/* Argument[15..8] */
	xmit_spi((PFS_BYTE)arg);				/* Argument[7..0] */
...
DRESULT disk_readp (
	PFS_BYTE *buff,		/* Pointer to the read buffer (NULL:Forward to the stream) */
	PFS_DWORD sector,	/* Sector number (LBA) */
	PFS_UINT offset,	/* Byte offset to read from (0..511) */
	PFS_UINT count		/* Number of bytes to read (ofs + cnt mus be <= 512) */
)
{
	DRESULT res;
	PFS_BYTE rc;
...
#if _USE_WRITE
DRESULT disk_writep (
	const PFS_BYTE *buff,	/* Pointer to the bytes to be written (NULL:Initiate/Finalize sector write) */
	PFS_DWORD sc			/* Number of bytes to send, Sector number (LBA) or zero */
)
{
	DRESULT res;
	PFS_UINT bc;
	static PFS_UINT wc;	/* Sector write counter */
